        ...
        <div className={styles["desktop-content"]}>
          {listItems.map((listItem, i) => {
            return (
              <div
                key={i}
                ref={i === 0 ? null : (el) => (textDivRefs.current[i] = el)}
                className={styles["list-item-main"]}
              >
                <p>{listItem.label}</p>
                <h2>{listItem.title}</h2>
                <p>{listItem.desc}</p>
                {isProject && (
                  <Link to={`/projects/${listItem.id}`}>
        ...
        <div className={styles["desktop-images"]}>
          {listItems.map((listItem, i) => {
            return (
              <div
                key={i}
                ref={i === 0 ? null : (el) => (imgDivRefs.current[i] = el)}
                className={styles["desktop-list-item-img"]}
              >
